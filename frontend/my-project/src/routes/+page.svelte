<script>
  ...
  const fetchdata = () => {
    let sortQuery = sortBypublication
      ...
      : "";
    let sortQuery2 = sortBy
      ? `&ordering=${sortBy === "asc" ? "price" : "-price"}`
      : "";

    axios
      .get(
        `http://127.0.0.1:8000/api/books/?search=${serach}&page=${currentpage}&limit=${limit}&min_price=${minprice}&max_price=${maxprice}${sortQuery}${sortQuery2}`,
        {
          headers: {
  ...
  const updatedata = (id) => {
    axios
      .patch(`http://127.0.0.1:8000/api/books/${id}/`, updatedBookData, {
        headers: {
      ...
      .then((res) => {
        ...
        warning = false;
        showAlert = true;
        (Alertmassage = "Book updated successfully!"), (showModal = false);
      })
      .catch((err) => {
        console.log(err);
        showAlert = true;
  ...
  const handelsearch = () => {
    axios
      .get(`http://127.0.0.1:8000/api/books/?search=${serach}`, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
...
<div class="w-full flex">
  <div class="w-1/4 p-1">
    ...
    <div class="w-10/12 mt-2 p-2">
      ...
      <select
        class="w-full mt-2 border-1 rounded-2xl p-2"
        bind:value={filterbyAuthor}
        on:change={handelfilterbyauthor}
      >
    ...
    <div class="w-10/12 mt-2 p-2">
      ...
      <select
        class="w-full border-2 text-center rounded-3xl p-2"
        bind:value={sortBy}
        on:change={() => fetchdata()}
      >
    ...
    <div class="w-10/12 mt-2 p-2 text-center">
      <button class="p-1 bg-cyan-300 rounded-2xl" on:click={handleclearfilter}
  ...
  <div class=" w-full m-auto">
    ...
    <div class=" w-full flex justify-between mt-3 mb-3">
      <div class=" w-full flex justify-center">
        <input
          bind:value={serach}
          placeholder="search"
          class="border-2 p-0.5 pl-2 mb-2 mt-2 rounded-2xl outline-0 focus:border-red-600 mr-3"
        />
        <button
          class=" bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl"
          on:click={handelsearch}>search</button
        >
      </div>
    </div>
    {#if data.length === 0}
          ...
          {#each data as item, i}
            <tr class="border-1 p-2 odd:bg-cyan-200 even:bg-amber-50">
              ...
              <td class="border-1 p-2">
                <button
                  class="p-1 pl-3 pr-3 bg-green-500 text-amber-50 m-1 ml-2 mr-2 rounded-xl"
                  on:click={() => update(item)}>Update</button
                >
                <button
                  class="p-1 pl-3 pr-3 bg-red-500 text-amber-50 m-1 ml-2 mr-2 rounded-xl"
                  on:click={() => handeldelete(item.id)}>Delete</button
                >
              </td>
            </tr>
          {/each}
    ...
    {#if showModal}
      <div
        class="fixed inset-0 flex items-center justify-center bg-opacity-30 backdrop-blur-md"
      >
        <div class="bg-white p-6 rounded-lg shadow-lg w-1/3">
          ...
          <div class="w-12/12 m-auto mt-5 p-1 justify-center">
            ...
            <input
              ...
              type="text"
              bind:value={updatedBookData.title}
              class="w-full border-b-2 p-0.5 pl-2 mb-2 mt-2 outline-0 focus:border-red-600"
            />
            ...
            <input
              ...
              type="text"
              bind:value={updatedBookData.author}
              class="w-full border-b-2 p-0.5 pl-2 mb-2 mt-2 outline-0 focus:border-red-600"
            />

            <label>Published Date</label>
            <input
              placeholder="book published_date"
              type="date"
              bind:value={updatedBookData.published_date}
              class="w-full border-b-2 p-0.5 pl-2 mb-2 mt-2 outline-0 focus:border-red-600"
            />
            ...
            <input
              ...
              min="1"
              type="number"
              bind:value={updatedBookData.price}
              class="w-full border-b-2 p-0.5 pl-2 mb-2 mt-2 outline-0 focus:border-red-600"
            />
            <div class="w-full mt-1 p-3 flex justify-between">
              <button
                class=" bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl"
                on:click={() => updatedata(updatedBookData.id)}>Update</button
              >
              <button
                class=" bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-xl"
                on:click={() => {
    ...
    <div class=" p-3 text-center mt-2">
      {#each Array(totalpage).fill(0) as _, index}
        <button
          class="px-4 py-2 rounded-xl border m-1 {currentpage == index + 1
            ? 'bg-amber-100'
            : 'bg-amber-300'}"
